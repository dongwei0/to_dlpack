...
libmain = ctypes.cdll.LoadLibrary("./libdlpack.so")
...
def make_dl_tensor(array):
  ...
  dl_tensor = DLTensor()
  array = mindspore.Tensor.asnumpy(array)
...
def _dlpack_deleter(pycapsule):
    ...
    if ctypes.pythonapi.PyCapsule_IsValid(pycapsule, _c_str_dltensor):
        ...
        ctypes.pythonapi.PyCapsule_SetDestructor(pycapsule, DeleterFunc(0))
...
def to_dlpack(Tesor):
  c_obj = DLManagedTensor()
  c_obj.dl_tensor = make_dl_tensor(Tesor)
  c_obj.manager_ctx = make_manager_ctx(Tesor)
  c_obj.deleter = dl_managed_tensor_deleter
  data = libmain.Give(c_obj)
  # ptr = ctypes.pythonapi.PyCapsule_GetPointer(c_obj, _c_str_dltensor)
  return PyCapsule_New(data,_c_str_dltensor,None)


if __name__ == "__main__":
  t1 = mindspore.Tensor(np.zeros([1, 2, 3]), mindspore.float32)
  # to_dlpack(mindspore.Tensor.asnumpy(t1))
  # print(to_dlpack(mindspore.Tensor.asnumpy(t1)))
  print(to_dlpack(t1))
